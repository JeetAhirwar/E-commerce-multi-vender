...
export const createEmailClient = ({ smtpHost, smtpPort, smtpUser, smtpPass, emailFrom }) =>
{
  ...
  if (!isMockMode)
  {
    try
    {
      transporter = nodemailer.createTransport({
        host: smtpHost,
        port: smtpPort,
        secure: parseInt(smtpPort, 10) === 465, // True if SSL port 465 is utilized
        auth: {
          user: smtpUser,
          pass: smtpPass,
        },
    ...
    {
      console.warn('[SMTP INIT WARNING] Failed to establish SMTP transporter. Forcing Console fallback mode.', error.message);
    }
  }
  ...
  const sendOTPEmail = async ({ toEmail, otp }) =>
  {
    ...
    // HTML mail design visual setups (Branded under Jeet Ahirwar marketplace)
    const htmlEmailContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; padding: 25px; border-radius: 8px; background-color: #ffffff;">
        <div style="text-align: center; border-bottom: 2px solid #3b82f6; padding-bottom: 15px; margin-bottom: 20px;">
          <h2 style="color: #1e3a8a; margin: 0; font-size: 24px; font-weight: bold; letter-spacing: 0.5px;">
